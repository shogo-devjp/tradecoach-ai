...
export interface ChallengeViewProps {
  // true の場合「YouTube Safe Capture Mode」で描画する：
  // ・アプリ内ナビゲーション（「戻る」リンク等）を表示しない
  // ・個人情報・開発者向け情報を構造的に含まない要素だけで構成する
  //   （本ページはもともとPaper Tradingの公開可能な数値のみを扱っており、
  //   　ファイルパス・APIキー・LINE情報・Git情報等は元から一切描画していない）
  // ・データ未確定（読み込み中・エラー・記録0件）の場合は詳細を出し分けず、
  //   　常に同一の中立的なプレースホルダーのみを表示する（fail-safe）
  captureMode: boolean;
}

export default function ChallengeView({ captureMode }: ChallengeViewProps) {
  ...
  // 撮影素材として公開してよい状態か（§1 fail-safe要件）。
  // 読み込み中・取得失敗・Daily Record0件のいずれかであれば「まだ収録に適さない」とみなし、
  // Capture Modeでは中立的なプレースホルダーのみを表示して詳細な状態を出し分けない。
  const safeForPublishing = !isLoading && !fetchFailed && latest !== null;

  return (
    <main
      id="challenge-capture-root"
      data-capture-mode={captureMode}
      data-safe-for-publishing={safeForPublishing}
      className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-950 to-black px-4 py-10"
    >
      <div className="mx-auto w-full max-w-4xl rounded-3xl border border-slate-800 bg-slate-900/90 p-6 shadow-2xl shadow-black/50 ring-1 ring-amber-500/10 sm:p-8">
        ...
        {captureMode && !safeForPublishing && (
          <div className="flex h-40 items-center justify-center text-sm text-slate-500">コンテンツを準備しています</div>
        )}

        {!captureMode && isLoading && (
